...
colnames(Exp)[2] <-  "Export_Dolar"
colnames(abv)[2] <- "ABV"

#Converter coluna em número
Exp$Export_Dolar <- parse_number(Exp$Export_Dolar)

#Coloca tudo em minusculo
Exp$Country <- str_to_lower(Exp$Country)

#Adicionar abreviação ao Renda per capta
Exp <- left_join(Exp, abv)

#Junta as duas tabelas
Dados <- left_join(Homicide_per_100000, Exp, by = "ABV")
...
write_csv(Dados, "16_Summary_Homicide_Export.csv")

#Gerendo Grafico Completo
Grafico <- Dados %>% ggplot(aes(Export_Dolar, HomMed, label = ABV, col = Continente)) + geom_point(size = 3) +
  geom_text(nudge_x = 5) + 
...
#Grafico sem Legenda 
Grafico <- Dados %>% ggplot(aes(Export_Dolar, HomMed, col = Continente)) + geom_point(size = 2)  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
...
Grafico <- Dados %>% ggplot(aes(Export_Dolar, HomMed, col = Continente)) + geom_point(size = 1)  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
...
GAF <- AF %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "red")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 4000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Africa") 
GAN <- AN %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "gold")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 70000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Norte")
GAS <- AS %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "darkgreen")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 10000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Sul")
GASI <- ASI %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "dimgray")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 30000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Asia")
GEU <- EU %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "coral")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 50000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Europa")
GOC <- OC %>% ggplot(aes(Export_Dolar, HomMed, label = ABV)) + geom_point(size = 3, col = "darkmagenta")  +
  scale_x_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
  geom_text(nudge_x = 10000000000) + 
  xlab("Exportacoes em US$") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
