...
juros <- read_csv("231rank-Commercial_bank_prime_lending_rate.csv")
...
juros$Date.of.Information <- NULL
juros$Rank <- NULL

#Acerta o nome dos atributos
colnames(juros)[2] <-  "Juros_Per"
colnames(abv)[2] <- "ABV"

#Coloca tudo em minusculo
juros$Country <- str_to_lower(juros$Country)

#Adicionar abreviação ao Renda per capta
juros <- left_join(juros, abv)

#Junta as duas tabelas
Dados <- left_join(Homicide_per_100000, juros, by = "ABV")
...
write_csv(Dados, "14_Summary_Homicide_prime_lending.csv")

#Gerendo Grafico Completo
Grafico <- Dados %>% ggplot(aes(Juros_Per, HomMed, label = ABV, col = Continente)) + geom_point(size = 3)  +
  geom_text(nudge_x = 5) + 
  xlab("Taxa de juros %") +
...
#Grafico sem Legenda 
Grafico <- Dados %>% ggplot(aes(Juros_Per, HomMed, col = Continente)) + geom_point(size = 2)  +
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
Grafico
...
Grafico <- Dados %>% ggplot(aes(Juros_Per, HomMed, col = Continente)) + geom_point(size = 1)  +
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
...
GAF <- AF %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "red")  +
  geom_text(nudge_x = 0.6) + 
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Africa") 
GAN <- AN %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "gold")  +
  geom_text(nudge_x = 0.6) + 
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Norte")
GAS <- AS %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "darkgreen")  +
  geom_text(nudge_x = 1.5) + 
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Sul")
GASI <- ASI %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "dimgray")  +
  geom_text(nudge_x = 1) + 
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Asia")
GEU <- EU %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "coral")  +
  geom_text(nudge_x = 0.6) + 
  xlab("Taxa de juros %") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Europa")
GOC <- OC %>% ggplot(aes(Juros_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "darkmagenta")  +
