...
desemp <- read_csv("373rank-Unemployment,_youth_ages_15-24.csv")
...
colnames(desemp)[2] <-  "Desempregados_Jovens_Per"
colnames(abv)[2] <- "ABV"
...
write_csv(Dados, "12_Summary_Homicide_young_Unemployment.csv")

#Gerendo Grafico Completo
Grafico <- Dados %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV, col = Continente)) + geom_point(size = 3)  +
  geom_text(nudge_x = 5) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
Grafico

#Grafico sem Legenda 
Grafico <- Dados %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, col = Continente)) + geom_point(size = 2)  +
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
Grafico
...
Grafico <- Dados %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, col = Continente)) + geom_point(size = 1)  +
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo")
...
GAF <- AF %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "red")  +
  geom_text(nudge_x = 1.8) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Africa") 
GAN <- AN %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "gold")  +
  geom_text(nudge_x = 1.5) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Norte")
GAS <- AS %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "darkgreen")  +
  geom_text(nudge_x = 0.8) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - America do Sul")
GASI <- ASI %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "dimgray")  +
  geom_text(nudge_x = 1.5) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Asia")
GEU <- EU %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "coral")  +
  geom_text(nudge_x = 2) + 
  xlab("Taxa de Desempregados jovens (%)") +
  ylab("Taxa de homicidios por 100.000 habitantes") +
  ggtitle("Mortes por arma de fogo - Europa")
GOC <- OC %>% ggplot(aes(Desempregados_Jovens_Per, HomMed, label = ABV)) + geom_point(size = 3, col = "darkmagenta")  +
